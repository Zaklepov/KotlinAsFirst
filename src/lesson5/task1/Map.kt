...
fun subtractOf(a: MutableMap<String, String>, b: Map<String, String>) {
    ...
    for (key in a.keys) {
        if (a[key] == b[key] && b[key] != null) remove.add(key)
    }
    for (name in remove) {
        a.remove(name)
    }
...
fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> {
    ...
    val del = mutableMapOf<String, Double>()
...
fun hasAnagrams(words: List<String>): Boolean {
    val wordsset = mutableSetOf<List<Char>>()
    for (element in words) {
        wordsset.add(element.toList().sorted())
    }
    return words.size != wordsset.size
}
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    ...
    return if (result1 != -1 && result2 != -1 && result1 != result2) {
        ...
        if (firstI != -1 && secondI != -1) Pair(firstI, secondI)
        else Pair(-1, -1)
    } else Pair(-1, -1)
}
